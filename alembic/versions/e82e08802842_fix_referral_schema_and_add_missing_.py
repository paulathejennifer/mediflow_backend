...
#    op.drop_table('_alembic_tmp_facilities', if_exists=True)
    ...
    # Defensive upgrade for patient_identifiers
    pi_columns = [c['name'] for c in inspector.get_columns('patient_identifiers')]
    pi_constraints = [const['name'] for const in inspector.get_unique_constraints('patient_identifiers')]
    ...
    with op.batch_alter_table('patient_identifiers', schema=None) as batch_op:
        ...
        if 'mrn' in pi_columns:
            if 'unique_facility_mrn' not in pi_constraints:
                batch_op.create_unique_constraint('unique_facility_mrn', ['facility_id', 'mrn'])
        else:
            print("Warning: Column 'mrn' not found in 'patient_identifiers'. Skipping unique constraint.")
...
def downgrade() -> None:
    ...
    # Defensive downgrade for patient_identifiers
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    pi_constraints = [const['name'] for const in inspector.get_unique_constraints('patient_identifiers')]

    with op.batch_alter_table('patient_identifiers', schema=None) as batch_op:
        if 'unique_facility_mrn' in pi_constraints:
