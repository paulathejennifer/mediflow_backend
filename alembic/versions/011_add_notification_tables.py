...
def upgrade():
    ...
    if not conn.dialect.has_table(conn, 'notifications'):
        op.create_table(
            ...
            sa.Column('backend_source', sa.String(length=255), nullable=False, server_default='system'),
    ...
    if not conn.dialect.has_table(conn, 'notification_deliveries'):
        op.create_table(
            ...
            sa.Column('delivery_method', sa.String(length=255), nullable=False, server_default='websocket'),
            sa.Column('delivery_status', sa.String(length=255), nullable=False, server_default='pending'),
    ...
    if not conn.dialect.has_table(conn, 'notification_templates'):
        op.create_table(
            ...
            sa.Column('default_roles', sa.JSON(), nullable=False, server_default='[]'),
            sa.Column('backend_source', sa.String(length=255), nullable=False, server_default='system'),
    ...
    if not conn.dialect.has_table(conn, 'notification_preferences'):
        op.create_table(
            ...
            sa.Column('delivery_methods', sa.JSON(), nullable=False, server_default='["websocket"]'),
            sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=True),
            sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        )
    ...
    if not conn.dialect.has_table(conn, 'notification_queue'):
        op.create_table(
            ...
            sa.Column('status', sa.String(length=255), nullable=False, server_default='pending'),
