...
async def suspend_user_action(
    ...
    if not violating_user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Violating user not found"
        )
            ...
            try:
                ...
                if referral:
                    details["referral_details"] = {
                        ...
                        "patient_name": f"{referral.patient.first_name} {referral.patient.last_name}",
                        "from_facility": referral.from_facility.name if referral.from_facility else "Unknown",
                        "to_facility": referral.to_facility.name if referral.to_facility else "Unknown"
                    }
...
async def review_backlog_action(
    ...
    for referral in overdue_referrals:
        age_hours = (datetime.now(timezone.utc) - referral.created_at).total_seconds() / 3600
        ...
        backlog_details.append({
            ...
            "from_facility": referral.from_facility.name if referral.from_facility else "Unknown",
            "to_facility": referral.to_facility.name if referral.to_facility else "Unknown",
            "reason_for_referral": referral.reason_for_referral[:100] + "..." if len(referral.reason_for_referral) > 100 else referral.reason_for_referral
        })
    
    return {
        "backlog_count": len(backlog_details),
        "overdue_count": len([r for r in backlog_details if r["age_hours"] > 48]),
        "urgent_count": len([r for r in backlog_details if r["priority"] == "emergency"]),
