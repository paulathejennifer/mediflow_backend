...
from app.schemas.referral import ReferralCreate, ReferralUpdate, ReferralResponse, ReferralSummary, ReferralWithDetails
...
def create_referral(
    ...
    try:
        ...
        to_facility = db.query(Facility).filter(Facility.id == referral_data.to_facility_id).first()
        if not to_facility:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
def list_referrals(
    ...
    for referral in referrals:
        ...
        from_facility = db.query(Facility).filter(Facility.id == referral.from_facility_id).first()
        to_facility = db.query(Facility).filter(Facility.id == referral.to_facility_id).first()
