...
class TextCleaner:
    ...
    def clean_transcript(
        ...
        if context:
            text, context_corrections = TextCleaner._apply_context_corrections(
                text, context
            )
    ...
    def _correct_ocr_errors(text: str) -> Tuple[str, List[str]]:
        ...
        corrections.extend(
            TextCleaner._apply_corrections(
                text, TextCleaner.OCR_CORRECTIONS, "OCR correction"
            )
        )
        ...
        corrections.extend(
            TextCleaner._apply_corrections(
                text, medical_ocr_errors, "Medical OCR correction"
            )
        )
    ...
    def _fix_transcription_errors(text: str) -> List[str]:
        ...
        corrections.extend(
            TextCleaner._apply_corrections(
                text, transcription_fixes, "Transcription correction"
            )
        )
        ...
        if patient_name and len(patient_name) > 3:
            ...
            for variation in name_variations:
                if variation.lower() in text.lower():
                    text = re.sub(
                        re.escape(variation), patient_name, text, flags=re.IGNORECASE
                    )
    ...
    def _apply_corrections(
        text: str, corrections: Dict[str, str], correction_type: str
    ) -> List[str]:
    ...
    def _calculate_text_quality(original: str, cleaned: str) -> float:
        ...
        med_terms = list(TextCleaner.MEDICATION_ABBREVIATIONS.keys()) + list(
            TextCleaner.COMMON_MEDICAL_TERMS.keys()
        )
