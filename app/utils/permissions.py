...
class PermissionChecker:
    ...
    def check_referral_access(self, referral_id: int) -> bool:
        ...
        if (
            referral.from_facility_id != self.current_user.facility_id
            and referral.to_facility_id != self.current_user.facility_id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    def check_user_management(
        self, target_user_id: int = None, target_facility_id: int = None
    ) -> bool:
        ...
        if self.current_user.role == UserRole.FACILITY_ADMIN:
            ...
            if (
                target_facility_id
                and target_facility_id != self.current_user.facility_id
            ):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
            ...
            if target_user_id:
                ...
                if target_user and target_user.role in [
