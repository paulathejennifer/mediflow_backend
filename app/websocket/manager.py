...
class ConnectionManager:
    ...
    async def disconnect(self, user_id: str):
        ...
        if user_id in self.active_connections:
            ...
            if (
                user_role in self.role_connections
                and user_id in self.role_connections[user_role]
            ):
    ...
    async def broadcast_to_role(self, role: str, notification: dict):
        ...
        if role in self.role_connections:
            ...
            logger.info(
                f"Broadcast to role {role}: {success_count} sent, {failed_count} failed"
            )
    ...
    async def broadcast_to_roles(self, roles: List[str], notification: dict):
        ...
        logger.info(
            f"Broadcast to roles {roles}: {total_sent} sent, {total_failed} failed"
        )
    ...
    async def broadcast_to_facility(self, facility_id: int, notification: dict):
        ...
        if facility_id in self.facility_connections:
            ...
            logger.info(
                f"Broadcast to facility {facility_id}: {success_count} sent, {failed_count} failed"
            )
            return success_count, failed_count
        else:
    ...
    async def send_pending_notifications(self, user_id: int):
        ...
        try:
            ...
            logger.info(
                f"Sent {len(undelivered_notifications)} pending notifications to user {user_id}"
            )
