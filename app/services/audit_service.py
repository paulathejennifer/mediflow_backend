...
class AuditService:
    ...
    def get_audit_logs(self, user_id: Optional[int] = None, entity_type: Optional[str] = None,
    ...
    def get_user_activity_summary(self, user_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        logs = (
            ...
            .filter(and_(AuditLog.user_id == user_id, AuditLog.created_at >= start_date))
            .all()
        )
    ...
    def get_entity_history(self, entity_type: str, entity_id: int) -> List[Dict[str, Any]]:
        ...
        logs = (
            self.db.query(AuditLog)
            .filter(and_(AuditLog.entity_type == entity_type, AuditLog.entity_id == entity_id))
        ...
        users = {user.id: user for user in self.db.query(User).filter(User.id.in_(user_ids)).all()}
        ...
        return [
            {
                ...
                } if log.user_id in users else None,
                "details": log.details,
                "ip_address": log.ip_address,
    ...
    def get_compliance_report(self, facility_id: Optional[int] = None, days: int = 30) -> Dict[str, Any]:
        """
        ...
        user_creation_logs = [log for log in logs if log.action == AuditAction.CREATE.value and log.entity_type == "user"]
        ...
        download_logs = [log for log in logs if log.action == AuditAction.DOWNLOAD.value]
        ...
        return {
            ...
            "security_metrics": {
                ...
                "login_success_rate": (len(login_logs) / max(len(login_logs) + len(failed_logs), 1)) * 100
            },
            "data_access_metrics": {
                "patient_record_access": len(patient_access_logs),
                "referral_access": len(referral_access_logs),
                "total_data_access": len(patient_access_logs) + len(referral_access_logs)
            },
            "data_modification_metrics": {
                "records_created": len(create_logs),
                "records_updated": len(update_logs),
                "records_deleted": len(delete_logs),
                "total_modifications": len(create_logs) + len(update_logs) + len(delete_logs)
            },
            "file_operations": {
    ...
    def export_audit_logs(self, format: str = "json", filters: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        if format.lower() == "json":
            ...
            for log in logs:
                writer.writerow([
                    log.id, log.user_id, log.action, log.entity_type, 
                    log.entity_id, log.details, log.ip_address, 
