...
class VoiceService:
    ...
    async def upload_voice_note(self, referral_id: int, file: UploadFile, uploader_id: int) -> VoiceNote:
        """
        ...
        try:
            ...
            audio_metadata = await audio_handler.handle_upload(file, referral_id, uploader_id)
    ...
    def get_referral_voice_notes(self, referral_id: int, status: Optional[str] = None) -> List[VoiceNote]:
        """
    ...
    def update_voice_note(self, voice_note_id: int, transcript: str, 
                         processed_transcript: str = None, status: str = None,
                         ai_summary: str = None) -> VoiceNote:
        """
    ...
    def delete_voice_note(self, voice_note_id: int, deleter_id: int) -> bool:
        ...
        if (voice_note.uploaded_by != deleter_id and 
            deleter.role not in [UserRole.SUPER_ADMIN, UserRole.FACILITY_ADMIN]):
            raise HTTPException(
    ...
    def get_voice_note_summary(self, voice_note_id: int) -> Dict[str, Any]:
        ...
        # Get referral info
        referral = self.db.query(Referral).filter(Referral.id == voice_note.referral_id).first()
        ...
        return {
            ...
            "transcription_info": {
                ...
                "transcript_length": len(voice_note.transcript) if voice_note.transcript else 0,
                "has_processed_transcript": bool(voice_note.processed_transcript),
                "processed_transcript_length": len(voice_note.processed_transcript) if voice_note.processed_transcript else 0,
                "ai_summary": voice_note.ai_summary
            },
            "uploader_info": {
                "id": uploader.id,
                "name": f"{uploader.first_name} {uploader.last_name}" if uploader else "Unknown",
    ...
    def search_voice_notes(self, facility_id: int, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        """
        ...
        for vn in voice_notes:
            results.append({
                ...
                "matched_content": self._get_matched_content(vn.processed_transcript, query)
            })
    ...
    def get_voice_stats(self, facility_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_file_size_mb": round((total_size / max(total_voice_notes, 1)) / (1024 * 1024), 2)
        }
    ...
    def _get_matched_content(self, text: str, query: str, context_length: int = 100) -> str:
