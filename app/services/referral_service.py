...
class ReferralService:
    ...
    def create_referral(
        self, referral_data: ReferralCreate, creator_id: int
    ) -> Referral:
        """
        ...
        patient = (
            ...
            .filter(Patient.id == referral_data.patient_id)
            .first()
        )
        if not patient:
            raise HTTPException(
    ...
    def update_referral(
        self, referral_id: int, referral_update: ReferralUpdate, updater_id: int
    ) -> Referral:
        """
        ...
        if any(
            field in update_data for field in ["clinical_notes", "reason_for_referral"]
        ):
            self._trigger_ai_processing(referral.id)
    ...
    def reject_referral(
        self, referral_id: int, rejecter_id: int, reason: str
    ) -> Referral:
        """
    ...
    def get_referral_summary(self, referral_id: int) -> Dict[str, Any]:
        ...
        # Get related data
        patient = (
            self.db.query(Patient).filter(Patient.id == referral.patient_id).first()
        )
        from_facility = (
            self.db.query(Facility)
            .filter(Facility.id == referral.from_facility_id)
            .first()
        )
        to_facility = (
            self.db.query(Facility)
            .filter(Facility.id == referral.to_facility_id)
            .first()
        )
        ...
        voice_notes = (
            self.db.query(VoiceNote).filter(VoiceNote.referral_id == referral_id).all()
        )
    ...
    def get_referral_analytics(
        self, facility_id: int, days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "acceptance_rate": (
                status_breakdown.get(ReferralStatus.ACCEPTED.value, 0)
                / max(status_breakdown.get(ReferralStatus.SUBMITTED.value, 1), 1)
            )
            * 100,
        }
    ...
    def _trigger_ai_processing(self, referral_id: int) -> None:
        ...
        try:
            ...
            context = {
                ...
                "emergency_contact_name": referral_summary["patient_info"].get(
                    "emergency_contact_name", "None"
                )
                if referral_summary["patient_info"]
                else "None",
                "emergency_contact_phone": referral_summary["patient_info"].get(
                    "emergency_contact_phone", "None"
                )
                if referral_summary["patient_info"]
                else "None",
                "medical_history": referral_summary["patient_info"].get(
                ...
                "chronic_conditions": referral_summary["patient_info"][
                    "chronic_conditions"
                ]
                if referral_summary["patient_info"]
                else "None",
                ...
                "from_facility": referral_summary["facility_info"]["from_facility"][
                    "name"
                ]
                ...
                "documents_summary": self._summarize_documents(
                    referral_summary["attachments"]["documents"]
                ),
                "voice_transcripts": self._summarize_voice_notes(
    ...
    def _summarize_documents(self, documents: List[Dict]) -> str:
        ...
        for doc in documents:
            ...
            if text:
                ...
                summary.append(
                    f"- {doc['file_name']} ({doc['file_type']}): {truncated}"
                )
            else:
                summary.append(
                    f"- {doc['file_name']} ({doc['file_type']}): Not transcribed"
                )
