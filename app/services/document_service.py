...
class DocumentService:
    ...
    async def upload_document(self, referral_id: int, file: UploadFile, 
                            file_type: str, uploader_id: int) -> ReferralDocument:
        """
        ...
        try:
            ...
            file_metadata = await document_handler.handle_upload(file, referral_id, uploader_id)
    ...
    def get_document_by_id(self, document_id: int) -> Optional[ReferralDocument]:
        """Get document by ID."""
        return self.db.query(ReferralDocument).filter(ReferralDocument.id == document_id).first()

    def get_referral_documents(self, referral_id: int, file_type: Optional[str] = None) -> List[ReferralDocument]:
        """
        ...
        Returns:
            List of documents
        """
        query = self.db.query(ReferralDocument).filter(ReferralDocument.referral_id == referral_id)
    ...
    def delete_document(self, document_id: int, deleter_id: int) -> bool:
        ...
        if (document.uploaded_by != deleter_id and 
    ...
    def update_document_metadata(self, document_id: int, extracted_text: str, 
                                ai_processed: bool = True) -> ReferralDocument:
        """
    ...
    def get_document_summary(self, document_id: int) -> Dict[str, Any]:
        ...
        # Get referral info
        referral = self.db.query(Referral).filter(Referral.id == document.referral_id).first()
    ...
    def search_documents(self, facility_id: int, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        """
        ...
        for doc in documents:
            results.append({
                ...
                "matched_content": self._get_matched_content(doc.extracted_text, query)
            })
