...
class UserService:
    ...
    def create_user(self, user_data: UserCreate, creator_id: int) -> User:
        ...
        if user_data.facility_id:
            facility = self.db.query(Facility).filter(Facility.id == user_data.facility_id).first()
            if not facility:
                raise HTTPException(
    ...
    def update_user(self, user_id: int, user_update: UserUpdate, updater_id: int) -> User:
        """
        ...
        if "facility_id" in update_data and update_data["facility_id"]:
            facility = self.db.query(Facility).filter(Facility.id == update_data["facility_id"]).first()
            if not facility:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail="Assigned facility not found"
    ...
    def list_facility_users(self, facility_id: int, skip: int = 0, limit: int = 100) -> List[User]:
        """
    ...
    def list_users_by_role(self, role: str, skip: int = 0, limit: int = 100) -> List[User]:
        """
        List users by role.
